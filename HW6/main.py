...
def parse_args():
    ...
    parser.add_argument('--gamma_s', default=0.00007, type=float)
    parser.add_argument('--gamma_c', default=0.00007, type=float)
    parser.add_argument('--centroid_method', default='kmeans++', 
                        choices=['kmeans++', 'random'], type=str)
...
if __name__ == '__main__':
    ...
    # U = None
    # try:
    #     U = np.load('unnormalized_U_matrix.npy')
    # except:
    #     print("unnormalized U matrix not found")
    # unnormalized_SC = SpectralClustering(mode='unnormalized', U=U)
    ...
    # U = None
    # try:
    #     U = np.load('normalized_U_matrix.npy')
    # except:
    #     print("normalized U matrix not found")
    # normalized_SC = SpectralClustering(mode='normalized', U=U) 
    normalized_SC = SpectralClustering(mode='normalized')
    for n_cluster in range(2, 5):
        print("-" * 40 + f"k = {n_cluster}" + "-" * 40)
        cluster_result, assigned_color = normalized_SC.fit(
                                            W, n_cluster=n_cluster, 
                                            centroid_method=args.centroid_method, 
                                            EPS=args.EPS, visualize=True)
        visualization = assigned_color.reshape(-1, height, width, assigned_color.shape[-1])
        save_gif(visualization, gif_path=args.gif_path, 
                gif_name=f"normalized_SC_{args.centroid_method}_{n_cluster}.gif", 
                duration=2)

        if n_cluster == 2:
            normalized_SC.plotEigenspaceLaplacian2D(
                            assigned_cluster=cluster_result, 
                            plot_path=args.plot_path, 
                            plot_name=f"normalized_SC_{args.centroid_method}_{n_cluster}.png")
        elif n_cluster == 3:
            normalized_SC.plotEigenspaceLaplacian3D(
                            assigned_cluster=cluster_result, 
                            plot_path=args.plot_path, 
                            plot_name=f"normalized_SC_{args.centroid_method}_{n_cluster}.png")